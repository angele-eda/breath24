  ...
  const NumberControl = ({ title, hint, colorClass, settingKey, min, max }) => (
    <div className="flex items-center justify-between rounded-lg border border-white/10 bg-slate-950/30 p-3">
      ...
      <div className="flex items-center gap-2.5">
        ...
        <span className="w-8 text-center text-sm font-bold tabular-nums text-white">{settings[settingKey]}초</span>
        <button
          onClick={() => adjustCustomPart(settingKey, 1, min, max)}
          className="flex h-8 w-8 items-center justify-center rounded-full bg-slate-800 text-sm font-bold text-slate-100 transition hover:bg-slate-700 active:scale-90"
        >
  ...
  return (
    <div className="w-full max-w-3xl mx-auto px-5 py-6 space-y-5 animate-fade-in">
      <div className="px-1">
        ...
        <h2 className="mt-3 text-2xl font-bold text-white">호흡 환경 설정</h2>
        <p className="mt-1 text-sm text-slate-500">소리, 시간, 맞춤 호흡 리듬을 편하게 조절하세요.</p>
      </div>
      ...
      <section className="overflow-hidden rounded-lg border border-white/10 bg-slate-900/60 text-slate-100 shadow-sm dark:bg-slate-800/50">
        <ToggleRow
          ...
          title="테마 모드"
          description="밝은 화면과 어두운 화면을 전환합니다."
          settingKey="darkMode"
        />
        ...
        <div className="border-b border-white/10 p-4">
          <div className="flex items-center gap-3.5">
            ...
            <div>
              <h4 className="text-sm font-bold text-slate-100">기본 세션 시간</h4>
              <p className="mt-0.5 text-[11px] text-slate-500">호흡 시작 시 적용되는 기본 시간을 정합니다.</p>
            </div>
          </div>
        ...
        <ToggleRow icon={<Volume2 className="h-5 w-5" />} title="종소리 안내" description="단계가 바뀔 때 부드러운 소리를 재생합니다." settingKey="soundCuesEnabled" />
        <ToggleRow icon={<Mic className="h-5 w-5" />} title="음성 안내" description="호흡 시작과 완료를 음성으로 안내합니다." settingKey="voiceCuesEnabled" />
        <div className="flex items-center justify-between gap-4 border-b border-white/10 p-4">
          <div className="min-w-0">
            <h4 className="text-sm font-bold text-slate-100">목소리</h4>
            <p className="mt-0.5 text-[11px] leading-4 text-slate-500">시작 안내에 사용할 목소리를 선택합니다.</p>
          </div>
        ...
        <ToggleRow icon={<Smartphone className="h-5 w-5" />} title="진동 효과" description="지원 기기에서 미세한 진동 피드백을 사용합니다." settingKey="vibrationCuesEnabled" />
      </section>
      ...
      <section className="rounded-lg border border-white/10 bg-slate-900/60 p-5 text-slate-100 shadow-sm dark:bg-slate-800/50">
        <div className="flex items-center gap-2 border-b border-white/10 pb-4">
          ...
          <h3 className="text-sm font-bold text-slate-100">사용자 맞춤 호흡 비율</h3>
        </div>
        <p className="mt-4 text-xs leading-5 text-slate-500">
          호흡법 선택에서 사용자 맞춤 호흡을 고르면 아래 비율로 세션이 진행됩니다.
        </p>

        <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
          <NumberControl title="1. 들이마시기" hint="추천: 4초" colorClass="text-teal-300" settingKey="customInhale" min={1} max={10} />
          <NumberControl title="2. 멈추기" hint="0초면 생략" colorClass="text-amber-300" settingKey="customHold" min={0} max={15} />
          <NumberControl title="3. 내쉬기" hint="추천: 6초" colorClass="text-sky-300" settingKey="customExhale" min={1} max={15} />
          <NumberControl title="4. 빈 채 멈추기" hint="0초면 생략" colorClass="text-indigo-300" settingKey="customHold2" min={0} max={10} />
        </div>
